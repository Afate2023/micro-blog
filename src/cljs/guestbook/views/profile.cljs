...
(rf/reg-sub :profile/changed?
            :<- [:profile/changes]
            (fn [changes _]
              (not (empty? changes))))
(rf/reg-sub :profile/field-changed?
            :<- [:profile/changes]
            (fn [changes [_ k]]
              (contains? changes k)))
(rf/reg-sub
...
(defn display-name [] (r/with-let [k :display-name value (rf/subscribe [:profile/field k ""])]
                        [:div.field
...
(defn bio [] (r/with-let [k :bio value (rf/subscribe [:profile/field k ""])]
               [:div.field
