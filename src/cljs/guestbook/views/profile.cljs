...
(rf/reg-sub
 ...
 (fn [[changes {:keys [profile]} media] [_ k default]]
   (or
    ...
    (get changes k) (get profile k) default)))
...
(defn display-name []
  (r/with-let [k :display-name value (rf/subscribe [:profile/field k ""])]
    [:div.field
...
(defn bio []
  (r/with-let [k :bio value (rf/subscribe [:profile/field k ""])]
    [:div.field
     [:label.label {:for k} "Bio"
      (when @(rf/subscribe [:profile/field-changed? k])
        " (Changed)")]
     [:div.control {:style {:margin-bottom "0.5em"}} [textarea-input {:value value
                                                                      :on-save #(rf/dispatch [:profile/save-change k %])}]]
     [:div.control>button.button.is-danger
      {:disabled (not @(rf/subscribe [:profile/field-changed? k]))
       :on-click #(rf/dispatch [:profile/save-change k nil])} "Reset"]]))
(defn avatar []
  (r/with-let [k :avatar url (rf/subscribe [:profile/field k ""])]
    [:<>
     [:h3 "Avatar"
      (when @(rf/subscribe [:profile/field-changed? k])
        " (Changed)")]
     ...
     [:div.field.is-grouped
      ...
      [:div.control>button.button.is-danger
       ...
       "Reset Avatar"]]]))
(defn banner []
  (r/with-let [k :banner url (rf/subscribe [:profile/field k ""])]
    [:<>
     [:h3 "Banner"
      (when @(rf/subscribe [:profile/field-changed? k])
        " (Changed)")]
...
(defn delete-account [username]
  (r/with-let [fields (r/atom {}) login (r/cursor fields [:login]) password (r/cursor fields [:password]) status (r/atom {})]
    [:<>
     [:h3 "Delete Account"]
     [m/modal-button
      ::delete-account
      ...
      [:div.field.is-grouped
       ...
       [:p.control>button.button.is-danger
        {:disabled (or (:loading @status) (empty? @login) (empty? @password))
         :on-click
         (fn [_]
           (if (not= @login username) (reset! status
                                              {:error
               ...
               (do
                 ...
                 (ajax/POST "/api/my-account/delete-account"
                   {:params @fields
                    ...
                    :error-handler
                    (fn [{r :response}]
                      (if (= (:error r) :incorrect-password) (reset! status {:error
                                                                             "Incorrect password, please try again."}) (reset! status {:error
